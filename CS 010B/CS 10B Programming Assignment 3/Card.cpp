...
Card::Card(char userSuit, int userRank)
{
    ...
    if (isupper(_suit))
    {
        _suit += 32; // to convert upper to lower by changing ASCII value
    }
    if (_suit != 'c' && _suit != 'd' && _suit != 'h' && _suit != 's') // to handle exceptions
    {
        _suit = 'c';
    }

    if (_rank > 13 || _rank < 1) // to handle exceptions
    {
        _rank = 2;
    }
}
