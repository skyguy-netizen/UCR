...
HashTable::HashTable (int s) {
    hashTable = new list<WordEntry>[s]; //create list of that size
    size = s;
}
...
void HashTable::put(const string &s, int score) {
    ...
    if (hashTable + h == nullptr){
        hashTable[h] = list<WordEntry>();
    }

    list<WordEntry> *li = hashTable + h;
    auto i = li->begin();
    while (i != li->end()){
        if (i->getWord() == s){
    ...
    li->push_back(WordEntry(s, score));
}
...
double HashTable::getAverage(const string &s) {
    if (contains(s)) {
        list<WordEntry> *li = hashTable + computeHash(s);
        auto i = li->begin();
        while (i != li->end()){
            WordEntry curr = *i;
            if (curr.getWord() == s)
...
bool HashTable::contains(const string &s) {
    ...
    int h = computeHash(s);
    list<WordEntry> *list = hashTable + h;
    auto i = list->begin();
    while (i != list->end()){
        WordEntry atI = *i;
        if (atI.getWord() == s){
